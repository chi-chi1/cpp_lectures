...
constexpr auto MAX_MODEL_MODEL= 64;
...
struct Inventory
{
        ...
        auto list()
        {
                std::printf("32s%64s%16s%8s\n", "Automobile", "Model Name", "Price (GBP)", "Qty.");
                std::for_each(cars.begin(), cars.end, [](const auto& car) {
                        std::printf("%32s%64s%16.2f%8d\n", get_automobile_name(car.id).dats(), car.name.cstr(), car.price, car.nstock);
                });
                std::printf("-------------\n");
        }
};
...
struct InventoryUI
{
        ...
        auto handle_add_option()
        {
                ...
                do {
                        ...
                        if (!is_valid_automobile(car.id)) {std::printf("Invalid option selected. Please try again.\n"); }
                        else
                        {
        ...
        auto handle_search_option()
        {
                ...
                if (pcar != Inventory::CarPtr {})
                {
                        do {
                                ...
                                {
                                        const auto new_car: const Car = handle_add_option();
                                        inventory.remove(pcar);
